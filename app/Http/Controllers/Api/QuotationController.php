...
class QuotationController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $quotations = $query->latest('quotation_date')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $quotation = DB::transaction(function () use ($validated, $request) {
            ...
            $quotationNumber = 'QUO-'.date('Ymd').'-'.str_pad(Quotation::whereDate('created_at', today())->count() + 1, 4, '0', STR_PAD_LEFT);
    ...
    {
        if ($quotation->tenant_id !== $request->user()->tenant_id) {
            abort(403, 'Unauthorized');
        }

        return response()->json($quotation->load(['customer', 'items.product']));
    }
